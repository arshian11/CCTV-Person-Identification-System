...
class YOLOLicense:
    ...
    def detect(self, frame):
        """
        Detect vehicles & plates, pair plates with closest vehicle.
        Returns list of:
            {
                "vehicle_type": "...",
        ...
        for box in det.boxes:
            ...
            vbox = box.xyxy[0].tolist()

            if name in ["car", "motorbike", "motorcycle", "bus", "truck"]:
                vehicles.append(vbox)
                labels.append(name)
            ...
            if crop.size > 0:
                try:
                    ...
                    if (
                        ...
                        and "rec_texts" in ocr_res[0]
                        and len(ocr_res[0]["rec_texts"]) > 0
                    ):
                        text = ocr_res[0]["rec_texts"][0]
    ...
    #             try:
    #                 if ocr_res and len(ocr_res[0]) > 0:
    #                     # OCR returns: [[ [bbox, (text, score)] , ... ]]
    #                     # Example: [[ [ [[0,0],[1,0],[1,1],[0,1]], ('ABC123', 0.98) ] ]]
    #                     plate_info = ocr_res[0][0]
    #                     if isinstance(plate_info, list) and len(plate_info) >= 2:
    #                         text = plate_info[1][0]  # extract text
    #             except Exception as e:
    #                 print("OCR parse error:", e, "OCR RESULT:", ocr_res)
